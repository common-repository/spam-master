...
if ( is_multisite() ) {
	...
	foreach ( $blogs as $idb ) {
		...
		$wpdb->query( "DELETE FROM $spam_master_keys WHERE spamkey = 'Option' AND spamtype = 'exempt-key' AND spamvalue = 'wc_order_attribution_type'" );

		// Update.
		update_blog_option( $idb, 'spam_master_upgrade_to_7_2_9', '1' );
	}
} else {
	// Update DB.
	...
	// Update.
	update_option( 'spam_master_upgrade_to_7_2_9', '1' );
}
...
$spamtype                   = 'Upgraded';
$spamvalue                  = 'Plugin Install or Upgrade Tasks Done 7_2_9.';
$cache                      = '4H';
