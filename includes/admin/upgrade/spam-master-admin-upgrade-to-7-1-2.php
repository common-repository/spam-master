...
if ( is_multisite() ) {
	...
	foreach ( $blogs as $idb ) {
		...
		$spam_master_firewall_die_pre = $wpdb->get_var( "SELECT spamvalue FROM {$spam_master_keys} WHERE spamkey = 'Option' AND spamtype = 'spam_master_firewall_die'" );
		if ( empty( $spam_master_firewall_die_pre ) ) {
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			$wpdb->insert(
				$spam_master_keys,
				array(
					...
					'spamkey'   => 'Option',
					'spamtype'  => 'spam_master_firewall_die',
					'spamy'     => 'localhost',
					'spamvalue' => 'true',
				)
			);
		...
		// Update.
		update_blog_option( $idb, 'spam_master_upgrade_to_7_1_2', '1' );
	}
} else {
	// Update DB.
	...
	// Update.
	update_option( 'spam_master_upgrade_to_7_1_2', '1' );
}
...
$spamtype                   = 'Upgraded';
$spamvalue                  = 'Plugin Install or Upgrade Tasks Done 7_1_2.';
$cache                      = '4H';
