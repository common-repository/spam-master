...
if ( is_multisite() ) {
	...
	foreach ( $blogs as $idb ) {
		...
		if ( empty( $spam_master_emails_alert_date ) ) {
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			$wpdb->insert(
				$spam_master_keys,
				array(
		...
		if ( empty( $spam_master_emails_alert_notice ) ) {
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			$wpdb->insert(
				$spam_master_keys,
				array(
		...
		// Update.
		update_blog_option( $idb, 'spam_master_upgrade_to_6_8_6', '1' );
	}
} else {
	// Update DB.
	...
	// Update.
	update_option( 'spam_master_upgrade_to_6_8_6', '1' );
}
...
$spamtype                   = 'Upgraded';
$spamvalue                  = 'Plugin Install or Upgrade Tasks Done 6_8_6.';
$cache                      = '4H';
