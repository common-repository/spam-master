...
if ( is_multisite() ) {
	...
	foreach ( $blogs as $idb ) {
		...
		$spam_master_license_sync_date = $wpdb->get_var( "SELECT spamvalue FROM {$spam_master_keys} WHERE spamkey = 'Option' AND spamtype = 'spam_master_license_sync_date'" );
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$spam_master_license_sync_run = $wpdb->get_var( "SELECT spamvalue FROM {$spam_master_keys} WHERE spamkey = 'Option' AND spamtype = 'spam_master_license_sync_run'" );
		...
		// Update.
		update_blog_option( $idb, 'spam_master_upgrade_to_6_8_5', '1' );
	}
} else {
	// Update DB.
	...
	// Update.
	update_option( 'spam_master_upgrade_to_6_8_5', '1' );
}
