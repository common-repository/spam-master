 ...
 * @since 4.0.1
 */
class SpamMasterAdminTableWhiteController extends WP_List_Table {
	...
	public function get_records( $per_page, $page_number ) {
		...
		wp_verify_nonce( 'spam-master-options-white', 'spam-master-options-white' );
		// Prepare sql.
		$spam_sql = "SELECT * FROM {$spam_master_keys} WHERE spamkey = 'White'";

		// Prepare open search.
		if ( ! empty( $_REQUEST['s'] ) ) {
			$spam_search  = wp_kses_post( wp_unslash( $_REQUEST['s'] ) );
			$search_white = esc_sql( $spam_search );
			$spam_sql    .= " AND (id LIKE '%" . $search_white . "%' OR time LIKE '%" . $search_white . "%' OR spamy LIKE '%" . $search_white . "%')";
		}
		...
		if ( ! empty( $records ) ) {
			foreach ( $records as $key ) {
				...
				if ( filter_var( $spamypre, FILTER_VALIDATE_IP ) ) {
					...
					if ( filter_var( $spamypre, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6 ) ) {
						...
						if ( filter_var( $spamypre, FILTER_VALIDATE_EMAIL ) ) {
							...
							$spamy = '<span class="dashicons dashicons-dismiss spam-master-admin-f32 spam-master-admin-red spam-master-middle" title="' . __( 'Unknown', 'spam-master' ) . '"></span>' . $spamypre;
						}
					}
				}
				$del_spamy = '<span class="dashicons dashicons-dismiss spam-master-admin-red spam-master-middle" title="' . __( 'Delete', 'spam-master' ) . '"></span>';
				...
				foreach ( $columns as $column_name => $column_display_name ) {
					...
					// Prepare deletion links, needs upper function.
					$time_nonce_now = current_time( 'H' );
					$link           = array(
						'page'           => 'spam-master.php&sm=white',
						'action'         => 'delete-white',
						'iddle'          => rawurlencode( $iddle ),
						'white_ip'       => rawurlencode( $spamypre ),
						'time_nonce_now' => rawurlencode( $time_nonce_now ),
					);
					$link           = add_query_arg( $link, admin_url( 'options-general.php' ) );
					$link           = wp_nonce_url( $link, "delete-white{$iddle}{$spamypre}{$time_nonce_now}" );

					// Display the cell.
					switch ( $column_name ) {
						case 'id':
							echo '<td class="' . esc_attr( $column_name ) . ' column-' . esc_attr( $column_name ) . ' spam-master-middle" style="text-align: left !important;">' . esc_html( $iddle ) . '</td>';
							break;
						case 'time':
						...
						case 'spamy':
							...
							break;
						case 'coldel':
							echo '<td class="' . esc_attr( $column_name ) . ' column-' . esc_attr( $column_name ) . ' spam-master-middle"><a href="' . esc_url( $link ) . '">' . wp_kses( $del_spamy, $selected_allowed ) . '</a></td>';
							break;
					}
				}
				echo '</tr>';
			...
			echo '<tr>';
			echo '<td colspan="4">' . esc_html( 'No buffer records found.' ) . '</td>';
			echo '</tr>';
		}
	}
	...
	public static function record_count() {
		...
		wp_verify_nonce( 'spam-master-options-white', 'spam-master-options-white' );
		if ( ! empty( $_REQUEST['s'] ) ) {
			$search_white = esc_sql( wp_kses_post( wp_unslash( $_REQUEST['s'] ) ) );
			$spam_sql     = "SELECT COUNT(*) FROM {$spam_master_keys}";
			$spam_sql    .= " WHERE spamkey = 'white' AND (id LIKE '%" . $search_white . "%' OR time LIKE '%" . $search_white . "%' OR spamy LIKE '%" . $search_white . "%')";
		} else {
			$spam_sql = "SELECT COUNT(*) FROM {$spam_master_keys} WHERE spamkey = 'White'";
		}
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
